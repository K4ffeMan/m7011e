...
import { authtest } from "../../backend/auth/jwtAuth";
import { pool, setupTestDatabase, cleanDatabase } from "../dbSetup";

// Wrap JWT middleware to satisfy TypeScript (Option B)
const getAuthMiddleware = (): RequestHandler =>
  authtest as unknown as RequestHandler;
...
async function createTodo(title: string, description: string, userId: number) {
  const result = await pool.query(
    "INSERT INTO todos (title, description, user_id) VALUES ($1, $2, $3) RETURNING todo_id",
...
async function markTodoComplete(todoId: number, userId: number) {
  const result = await pool.query(
    "UPDATE todos SET completed = TRUE WHERE todo_id = $1 AND user_id = $2 RETURNING *",
    [todoId, userId]
  );
